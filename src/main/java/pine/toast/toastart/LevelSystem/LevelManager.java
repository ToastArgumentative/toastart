  ...
  public static void addExperience(Player player, int amount) {
    ...
    if (experience + amount >= 105) {
      if (currentLevel >= maxLevel) {
        ...
        String className = player.getPersistentDataContainer().get(Keys.characterClass, PersistentDataType.STRING);
        Experience.buffs(player, currentLevel, className);

        if (currentLevel == 3 && CharacterClass.isMage(player)) player.sendMessage(ChatColor.GRAY + "You now have access to " + ChatColor.GREEN + "/skills" + ChatColor.GRAY + "! The more you level up, the more skills you unlock.");
        if (currentLevel == 5 && CharacterClass.isBarbarian(player)) player.sendMessage(ChatColor.GRAY + "You now have access to " + ChatColor.GREEN + "/skills" + ChatColor.GRAY + "! The more you level up, the more skills you unlock.");
        if (currentLevel == 4 && CharacterClass.isBomber(player)) player.sendMessage(ChatColor.GRAY + "You now have access to " + ChatColor.GREEN + "/skills" + ChatColor.GRAY + "! The more you level up, the more skills you unlock.");
      }
